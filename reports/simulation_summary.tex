...
Let us define a centering matrix \( M = I - \frac{1 1^T}{s} \) to express the MC in matrix notation. Since \( M \) is the centering matrix, \( [M y]_i = y_i - \bar{y} \).
...
If we flatten a 2D image into a 1D column vector \( \vec{x} \), an orthogonal transformation can be viewed as a rotation or reflection of this vector without changing its length.
\begin{proof}
  Suppose \( x \) is a column vector; its length can be expressed as \( x^T x \). Applying an orthogonal matrix \( Q \), which satisfies \( Q Q^T = I \), to \( x \), the length of the transformed vector \( Q x \) is \( (Q x)^T (Q x) = x^T x \).
\end{proof}
...
Suppose \( y \) is a column vector represents an image with 256 pixels.
