...
When dealing with spatially correlated data, like an image where pixel correlations depend on their relative positions, Moran’s Coefficient (MC) is a widely used measure of spatial autocorrelation. Suppose a vector \( x = (x_1, \ldots, x_s)^T \) represents an image with \( s \) pixels, and the spatial relationship between each pair of pixels is represented by a matrix \( C \), where \( C_{ij} \) indicates the spatial connection between pixels \( i \) and \( j \) (with diagonal elements equal to zero). According to \citet{griffith2014spatial}, the MC can be computed as follows:
\[
M C(x) = \frac{s}{\sum_{i=1}^s \sum_{j=1}^s c_{ij}} \cdot \frac{\sum_{i=1}^s \left(x_i - \bar{x} \right) \left[ \sum_{j=1}^s c_{ij} \left( x_j - \bar{x} \right) \right]}{\sum_{i=1}^s \left( x_i - \bar{x} \right)^2}
\]

\subsubsection*{Write Moran's coefficient in the matrix form}

To express Moran's coefficient (MC) in matrix form, we begin by defining the centering matrix \( M = I - \frac{1 1^T}{s} \), where \( I \) is the identity matrix and \( 1 \) is a column vector of ones. This matrix \( M \) is used to center the vector \( x \), ensuring it has a mean of zero. Specifically, applying \( M \) to \( x \) results in \( [M x]_i = x_i - \bar{x} \).
...
\]
\begin{align*}
  (M x)^T C (M x) & = \sum_{i=1}^s \sum_{j=1}^s (x_i - \bar{x}) c_{ij} (x_{j} - \bar{x}) \\
                  & = \sum_{i=1}^s (x_i - \bar{x}) \left( \sum_{j=1}^s c_{ij} (x_j - \bar{x}) \right)
\end{align*}
\begin{align*}
  x^T M x & = x^T M M x \\
          & = x^T M^T M x \\
...
\[
  M C(x) = \frac{s}{\vec{1}^T C \vec{1}} \cdot \frac{x^T M C M x}{x^T M x}
\]
...
We then perform the eigendecomposition of \( M C M \), which can be written as:
\[
  M C M = E \Lambda E^T
\]
where \( \Lambda \) is a diagonal matrix of eigenvalues, and \( E \) contains the corresponding eigenvectors as columns. We then transform the original image matrix \( X \) using the matrix of eigenvectors, 
\[
  X_{\text{freq}} = X \times E
\]
We call the original data is transformed into the ``frequency space'', which is a terminology frequently used in signal processing, reflecting the transformation decomposes data into spatially independent components, akin to frequencies, though here we are just reducing the correlation between pixels, not extinct them. To ensure the probability of \( y_i = 1 \) remain unchanged for all \( i \)s, assuming the coefficient vector in the frequency space is \( \vec{b} \), we can get its relationship with the \( \vec{\beta} \) as:
\[
  \vec{\beta} = E^T \vec{b}
\]
...
Since we hope that after whitening transformation, the correlation between the frequencies could be eliminated to almost zero. To simplify the simulation, we simulated \( X_{\text{freq}} \) by drawing 1000 samples from a muitivariate normal distribution with mean 0 and a diagonal covariance matrix of size \( 256 \times 256 \). Currently we are simulating the diagonal values to be decreasing with a constant step, but are open to more options. Then we randomly assign 10\% of values in \( \vec{b} \) to be a constant non-zero value \( b \), with all other values to be 0. The response vector \( y \) is simulated from binomial distributions independently, with the probability of \( y_i = 1 \) be \( p_i = 1 / ( 1 + \exp (- {X_{\text{freq}}}_{i \cdot} \vec{b} ) ) \).

We can calculate the corresponding predictors and coefficient vectors in the original space \( X \) and \( \vec{\beta} \) using the transformation stated above. Two logistic regression models with L1 penalty, one for \( X \) and another for \( X_{\text{freq}} \), will be fitted, and we will compare the model performance.
