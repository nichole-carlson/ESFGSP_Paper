...
\begin{proof}
  From \( \boldsymbol{z} = \boldsymbol{W} \boldsymbol{x} \) and \( \operatorname{cov} (\boldsymbol{z}) = W \Sigma W^T = I \), so \( W (\Sigma W^T W) = W \) and \( \Sigma W^T W = I \). Finally it gives us \( W^T W = \Sigma^{-1} \).
\end{proof}
...
\begin{proof}
  Here \( W^T W = U \Lambda^{-1/2} \Lambda^{-1/2} U^T = U \Lambda^{-1} U^T \). Since \( \Sigma \) is a covariance matrix, it is symmetric, so \( U \) is orthogonal meaning \( U U^T = I \), so \( U^T = U^{-1} \). Finally \( U \Lambda^{-1} U^T = (U^T)^{-1} (\Lambda^{-1})^{-1} U^{-1} = (U \Lambda U^T)^{-1} = \Sigma^{-1} \). 
\end{proof}

Actually we do not need a whitening matrix to achieve uncorrelation, for example, in the \( W \) built with eigen-decomposition above, \( W^{\prime} = U^T \) is enough to achieve this.
\begin{proof}
  \( cov(W^{\prime} x) = W^{\prime} \Sigma {W^{\prime}}^T = U^T \Sigma U = U^T U \Lambda U^T U = \Lambda \).
\end{proof}
\( \Lambda \) is a diagonal matrix with its diagonal values as eigenvalues. Thus we proved that using the eigenvectors of random vector's covariance matrix to perform linear transformation could resolve correlation between vector items, though not making them have constant variance.

The Moran Coefficient (MC) is the most common measurement of spatial autocorrelation. Given a spatial pattern (an image with \( n \) pixels for example) represented with a vector \( y = (y_1, \ldots, y_n)^T \), \( MC(y) \) is a value, with larger absolute values representing strong spatial patterns, and a value close to 0 means relative random pattern. It's expression can be written as \cite{Griffith_Chun_2014}:
\[
MC(y) = \frac{n}{\sum_{i=1}^n \sum_{j=1}^n c_{i j}} \frac{\sum_{i=1}^n\left(y_i-\bar{y}\right)\left[\sum_{j=1}^n c_{i j}\left(y_j-\bar{y}\right)\right]}{\sum_{i=1}^n\left(y_i-\bar{y}\right)^2}
\]
...
\[
\frac{n}{\mathbf{1}^{\prime} \mathbf{C} \mathbf{1}} \frac{\mathrm{y}^{\prime} \mathbf{M C M y}}{\mathbf{y}^{\prime} \mathbf{M y}}
\]
where \( \mathbf{M}=\mathbf{I}-11^{\prime} / n \) is called a centering matrix, as it centers vector \( y \) to mean 0.

Spatial filtering (\cite{Tiefelsdorf_Griffith_2007}) is a method [briefly describe what it does]. Eigenvector Spatial Filtering (ESF) is a method utilizes eigenvectors extracted from the spatial closeness matrix \( \mathbf{C} \). [Show how C could be defined.] The decomposition could be expressed as
\[
  \mathbf{M C M}=\mathbf{E} \boldsymbol{\Lambda} \mathbf{E}^{\mathrm{T}}
\]
where the diagonal values of \( \boldsymbol{\Lambda} \) are the eigenvalues in descending order. And each column of \( \mathbf{E} \) is the corresponding eigenvector. According to the property of eigenvectors, we know that \( \mathbf{e}_i^T \mathbf{e}_j = 0 \) if \( i \neq j \) otherwise 1. In matrix notation, this is \( \mathbf{E} \mathbf{E}^T = \mathbf{I} \). [Another property of the eigenvectors of this matrix I read about is \( E^T 1 = 0 \), which I know is correct but haven't verifed it]. Other properties we know about M is that it is idempotent, meaning \( \mathbf{M} \mathbf{M} = \mathbf{M} \). And the last property we need is \( E = ME \).

\begin{proof}
  Since E is the matrix of eigenvectors, we have 
\[
  MCM E = E \Lambda
\]
Then,
...
\[
  \begin{aligned}
    E \Lambda = M E \Lambda \\
    (E - ME) \Lambda = 0 \\
    (E - ME) \Lambda \Lambda^{-1} = 0 \\
    E = ME
  \end{aligned}
\]
\end{proof}

Then the point we try to make is the MC value of each eigenvector is proportional to its corresponding eigenvalue. THat is 
\[
  \text{MC}(\mathbf{e}_i) = \frac{n}{\mathbf{1}^T C \mathbf{1}} \lambda_i
\]

\begin{proof}
  \[
    \begin{aligned}
      \frac{n}{\mathbf{1}^{\prime} \mathrm{C} 1} \frac{\mathrm{e_i}^{\prime} \mathrm{MCMe_i}}{\mathrm{e_i}^{\prime} \mathrm{Me_i}} \\
      = \frac{n}{\mathbf{1}^{\prime} \mathrm{C} 1} \frac{e_i^{\prime} e_i \lambda_i}{\mathrm{e_i}^{\prime} \mathrm{Me_i}} \\
      = \frac{n}{\mathbf{1}^{\prime} \mathrm{C} 1} \frac{\lambda_i}{\mathrm{e_i}^{\prime} \mathrm{Me_i}} \\
      = \frac{n}{\mathbf{1}^{\prime} \mathrm{C} 1} \frac{\lambda_i}{e_i^{\prime} e_i} \\
      = \frac{n}{\mathbf{1}^{\prime} \mathrm{C} 1} \lambda_i
    \end{aligned}
  \]
\end{proof}
