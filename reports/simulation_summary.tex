...
\[
  M C(y) = \frac{n}{1}^{\prime} C 1 \cdot \frac{y^{\prime} M C M y}{y^{\prime} M y}
\]
...
\[
  M C M = E \Lambda E^{T}
\]
where \( \Lambda \) is a diagonal matrix of eigenvalues, and \( E \) contains the corresponding eigenvectors as columns.

Since \( M C M \) is symmetric, the eigenvectors are orthogonal \( \left(E E^{\prime}=I \right) \). Additionally, the eigenvectors corresponding to non-zero eigenvalues are orthogonal to the vector of ones, \( E^{\prime} 1 = 0 \), since \( M C M 1 = 0 \).

Now, we prove that for any eigenvector \( e_i \) corresponding to a non-zero eigenvalue \( \lambda_i \), it holds that \( M e_i = e_i \).

\begin{proof}
  Starting with the eigencomposition \( M C M = E \Lambda E^{T} \), multiply both sides by \( M \) from the left:

  \[
    M M C M E = M E \Lambda.
  \]

  Since \( M \) is idempotent \( (M^2 = M) \), this simplifies to:

  \[
    M C M E = M E \Lambda.
  \]

  Substituting the eigendecomposition \( M C M = E \Lambda E^T \), we get:

  \[
    E \Lambda = M E \Lambda.
  \]

  For any eigenvector \( e_i \) corresponding to \( \lambda_i \neq 0 \), it follows that:
...
Now that we've estabilished this property, we can show that the Moran coefficient for each eigenvector is proportional to its corresponding eigenvalue. The Moran coefficient of \( e_i \) is given by:

\[
  M C\left(e_i\right)=\frac{n}{1^{\prime} C 1} \frac{e_i^{\prime} M C M e_i}{e_i^{\prime} M e_i} .
\]

Using \( M C M e_i = \lambda_i e_i \) and \( M e_i = e_i \), this simplifies to:

\[
  M C\left(e_i\right)=\frac{n}{1^T C 1} \lambda_i.
\]
